...
function grade () {
    ...
    echo "++++++++++ RESULT ++++++++++"
    echo "$SUID" "$SUCC""/""$COUNT" "passed"
    echo "Score: $percent"/100 
    echo "SHA256: $SHA256"
    echo "$percent" > score
}
export -f grade
...
if [ $# != 4 ]
then
    echo "Welcome to the CMSI-585 homework script:
...
case $HOMEWORK in
    ...
    "hw2")
        MAINCLASS="Typecheck"
        LIBRARIES="classes;=;$LIB_FOLDER/tinyjava-parser.jar"
        GENRESULT="copy"
        ;;
    *)
